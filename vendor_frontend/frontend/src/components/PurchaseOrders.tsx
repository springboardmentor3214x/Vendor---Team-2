...
import { PurchaseOrder, Vendor, toINR } from "../data";
...
export function PurchaseOrders({ orders, vendors, currentRole, userVendorName, onAddOrder, onUpdateOrder }: POProps) {
  ...
  // Constraint: Only Approved AND Active vendors
  const eligibleVendors = vendors.filter(v => v.approvalStatus === "Approved" && v.status === "Active");
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)" }}>
          <form onSubmit={handleSubmit} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 740, maxHeight: "90vh", overflowY: "auto", border: "1px solid #E4E7EC", padding: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
              <div>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#374151", marginBottom: 4 }}>ASSIGN SUPPLIER (APPROVED & ACTIVE ONLY) *</label>
                <select value={vendorId} onChange={e => setVendorId(e.target.value)} required style={{ width: "100%", padding: 8, border: "1px solid #E4E7EC", borderRadius: 6 }}>
                ...
                {eligibleVendors.length === 0 && (
                  <span style={{ fontSize: 11, color: "#C62828" }}>Warning: No vendors exist with Approved check & Active status.</span>
                )}
              </div>
              <div>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#374151", marginBottom: 4 }}>PROCUREMENT CATEGORY</label>
                <select value={poCategory} onChange={e => setPoCategory(e.target.value)} style={{ width: "100%", padding: 8, border: "1px solid #E4E7EC", borderRadius: 6 }}>
