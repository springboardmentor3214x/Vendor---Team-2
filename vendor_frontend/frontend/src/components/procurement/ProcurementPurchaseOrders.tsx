...
import {
  ...
  CheckCircle, X, Truck, Package, AlertTriangle, Clock, FileText
} from "lucide-react";
...
function POCreateForm({ roleColor, currentRole, userName, prefillRequestId, onSuccess, onCancel }: {
  ...
  useEffect(() => {
    procurementService.getRequests({ status: "Approved", pageSize: 50 }).then(r => {
      const reqs = r.items.filter(x => x.assignedVendorName);
      setApprovedReqs(reqs);
      const pref = prefillRequestId ? reqs.find(x => x.id === prefillRequestId) : reqs[0];
      if (pref) applyRequest(pref);
      setLoading(false);
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [prefillRequestId]);

  const applyRequest = (r: ProcurementRequest) => {
    setSelectedReq(r);
  ...
  const validate = () => {
    ...
    if (!selectedReq) e.req = "Please select a procurement request.";
  ...
  const handleSubmit = async (draft = false) => {
    if (!validate() || !selectedReq) return;
    setSubmitting(true);
    try {
      const po = await procurementService.createPurchaseOrder({
        requestId: selectedReq.id,
        requestNumber: selectedReq.requestNumber,
        vendorId: selectedReq.assignedVendorId ?? 1,
        vendorName: selectedReq.assignedVendorName ?? "Unknown Vendor",
        productDetails,
        quantityOrdered,
        unitPrice,
        totalCost,
  ...
  if (loading) return <div style={{ padding: 40, textAlign: "center", color: "#9CA3AF" }}>Loading approved requests…</div>;
    ...
    <div style={{ padding: "24px 28px", fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <div>
          ...
          <span style={{ fontSize: 12, color: "#667085" }}>Create a PO from an approved vendor-assigned request</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 820 }}>
        <div style={{ ...card, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div>
            <label style={lbl}>PROCUREMENT REQUEST <span style={{ color: "#C62828" }}>*</span></label>
            <select
              value={selectedReq?.id ?? ""}
              onChange={e => {
                const r = approvedReqs.find(x => x.id === Number(e.target.value));
                if (r) applyRequest(r);
              }}
              style={{ ...inp(errors.req), cursor: "pointer" }}
            >
              <option value="">— Select an approved request —</option>
              {approvedReqs.map(r => (
                <option key={r.id} value={r.id}>{r.requestNumber} — {r.requestTitle} ({r.assignedVendorName})</option>
              ))}
            </select>
            {errors.req && <span style={{ fontSize: 11, color: "#C62828", display: "block", marginTop: 4 }}>{errors.req}</span>}
          </div>

          {selectedReq && (
            <div style={{ background: "#F9FAFB", borderRadius: 10, padding: "12px 16px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              <div><div style={{ fontSize: 9, color: "#9CA3AF", textTransform: "uppercase", fontWeight: 700, marginBottom: 2 }}>Vendor</div><div style={{ fontSize: 12, fontWeight: 700 }}>{selectedReq.assignedVendorName}</div></div>
