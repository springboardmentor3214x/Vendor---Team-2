...
import {
  ...
  Shield, LogOut, User, IndianRupee, Activity, Scale,
  Wallet, CheckCircle, RefreshCw, XCircle, AlertCircle,
  AlertTriangle, Play, Server, Lock, Award, MessageSquare,
  Star, History, ClipboardList, RotateCcw, FileBadge, FolderOpen, ShieldCheck, BarChart2
} from "lucide-react";
import {
...
import { VendorReliabilityComponent } from "./components/reliability/VendorReliability";
...
import { ProcurementManagerDashboard } from "./components/dashboard/ProcurementManagerDashboard";
...
import { notificationSimulatorService } from "./services/notificationSimulatorService";
...
const ROLE_SETTINGS: Record<Role, { color: string; bg: string; tabs: string[] }> = {
  "Administrator": {
    color: "#1565C0",
    bg: "#EEF4FF",
    tabs: ["dashboard", "vendors", "procurement", "purchase-orders", "contracts", "performance", "analytics", "reports", "notifications", "notifications-settings", "proc-dashboard", "proc-requests", "proc-purchase-orders", "proc-tracking", "proc-invoices", "perf-dashboard", "perf-delivery", "perf-quality", "perf-communication", "perf-service", "perf-history", "perf-ranking", "rel-dashboard", "rel-details", "rel-ranking", "rel-risk", "rel-trends", "rel-recommendations", "cc-repository", "cc-renewals", "cc-certifications", "cc-documents", "cc-compliance", "cc-notifications", "comm-messages", "comm-discussions", "comm-history", "comm-files", "comm-activity", "pm-dashboard", "vendor-dashboard", "admin-dashboard", "settings"]
  },
  "Procurement Manager": {
    color: "#2E7D32",
    bg: "#E8F5E9",
    tabs: ["dashboard", "vendors", "procurement", "purchase-orders", "contracts", "performance", "notifications", "notifications-settings", "proc-dashboard", "proc-requests", "proc-purchase-orders", "proc-tracking", "proc-invoices", "perf-dashboard", "perf-delivery", "perf-quality", "perf-communication", "perf-service", "perf-history", "perf-ranking", "rel-dashboard", "rel-details", "rel-ranking", "rel-risk", "rel-trends", "rel-recommendations", "cc-repository", "cc-renewals", "cc-certifications", "cc-documents", "cc-compliance", "cc-notifications", "comm-messages", "comm-discussions", "comm-history", "comm-files", "pm-dashboard", "settings"]
  },
  "Supply Chain Manager": {
    color: "#6A1B9A",
    bg: "#F9FAFB",
    tabs: ["dashboard", "performance", "analytics", "reports", "notifications", "notifications-settings", "proc-requests", "proc-tracking", "perf-dashboard", "perf-delivery", "perf-quality", "perf-communication", "perf-service", "perf-history", "perf-ranking", "comm-messages", "comm-discussions", "comm-history", "comm-files", "settings"]
  },
  "Vendor": {
    color: "#006064",
    bg: "#E0F7FA",
    tabs: ["dashboard", "profile", "purchase-orders", "contracts", "reports", "notifications", "notifications-settings", "proc-purchase-orders", "proc-tracking", "proc-invoices", "cc-repository", "cc-certifications", "cc-documents", "comm-messages", "comm-discussions", "comm-history", "comm-files", "vendor-dashboard", "settings"]
  },
  "Finance Officer": {
    color: "#E65100",
    bg: "#FFF3E0",
    tabs: ["dashboard", "purchase-orders", "invoices", "reports", "notifications", "notifications-settings", "proc-invoices", "proc-purchase-orders", "comm-messages", "comm-discussions", "comm-history", "comm-files", "settings"]
  },
  "Auditor": {
    color: "#B71C1C",
    bg: "#FFEBEE",
    tabs: ["dashboard", "reports", "auditors", "notifications", "notifications-settings", "proc-requests", "cc-repository", "cc-certifications", "cc-compliance", "cc-notifications", "comm-messages", "comm-discussions", "comm-history", "comm-files", "comm-activity", "settings"]
  }
};
...
export default function App() {
  ...
  const [contractUnreadCount, setContractUnreadCount] = useState(0);
  ...
  // Poll contract & communication notification counts every 30s
  useEffect(() => {
    const syncCount = () => {
      contractService.getUnreadCount().subscribe(n => setContractUnreadCount(n));
      communicationService.getUnreadNotificationCount().subscribe(n => setCommUnreadCount(n));
    };
    syncCount();
  ...
  const handleLoginSuccess = (r: Role, email: string, name: string) => {
    ...
    // Start live event simulator after login
    notificationSimulatorService.start(r);
  };

  const handleLogout = () => {
    localStorage.removeItem("token");
    notificationSimulatorService.stop();
    setScreen("role-select");
  };

  // Sync simulator role when activeRole changes
  useEffect(() => {
    if (screen === "auth-app") {
      notificationSimulatorService.setRole(activeRole as "Administrator" | "Procurement Manager" | "Supply Chain Manager" | "Vendor" | "Finance Officer" | "Auditor");
    }
  }, [activeRole, screen]);

  // Run startup scheduler when authenticated (check contracts + overdue POs)
  useEffect(() => {
    if (screen === "auth-app" && contracts.length > 0) {
      notificationSimulatorService.runStartupChecks(contracts, orders);
    }
  ...
  const handleUpdateVendor = (v: Vendor) => {
    ...
    const prevApproval = vendors.find(item => item.id === v.id)?.approvalStatus;
    const isApprovalUpdate = prevApproval !== v.approvalStatus;
    ...
    // Cross-module notification trigger
    if (isApprovalUpdate && v.approvalStatus === "Approved") {
      notificationService.addNotification({
        notificationType: 'VENDOR_APPROVAL',
        title: `Vendor Registration Approved — ${v.name}`,
        description: `Your vendor profile for ${v.name} has been approved by ${userName} (${activeRole}). You are now eligible to participate in procurement bids and receive Purchase Orders.`,
    ...
    } else if (isApprovalUpdate && v.approvalStatus === "Rejected") {
      notificationService.addNotification({
        notificationType: 'VENDOR_REJECTION',
        title: `Vendor Application Rejected — ${v.name}`,
        description: `The registration for ${v.name} has been rejected by ${userName} (${activeRole}). Please review the rejection reason and resubmit with the required documentation.`,
        relatedModule: 'Vendor Management',
        relatedRecordId: v.id,
        userId: v.id,
        priority: 'HIGH',
  ...
  const handleAddOrder = (po: PurchaseOrder) => {
    ...
    // Cross-module notification trigger: PO created → Vendor
    notificationService.addNotification({
      notificationType: 'PO_CREATED',
      title: `New Purchase Order Issued — ${po.id}`,
      description: `A new Purchase Order (${po.id}) for ${toINR(po.amount)} has been issued to ${po.vendorName} for ${po.category}. Please confirm acceptance within 48 hours via the portal.`,
      relatedModule: 'Procurement',
      relatedRecordId: po.id,
      userId: po.vendorId || 'vendor',
      priority: 'MEDIUM',
      deliveryMethod: ['IN_APP', 'EMAIL'],
    });
  };
  ...
  const handleUpdateOrder = (po: PurchaseOrder) => {
    ...
    // Cross-module: PO approved → fire notification
    if (po.status === 'Approved') {
      notificationService.addNotification({
        notificationType: 'PROCUREMENT_ALERT',
        title: `Purchase Order Approved — ${po.id}`,
        description: `PO ${po.id} for ${po.vendorName} has been approved and released. Value: ${toINR(po.amount)}. Delivery expected on ${po.deliveryDate ?? 'TBD'}. The vendor has been notified.`,
        relatedModule: 'Procurement',
        relatedRecordId: po.id,
        userId: 'proc-manager',
        priority: 'MEDIUM',
  ...
  const handlePayInvoice = (id: string) => {
    ...
    // Cross-module notification trigger: invoice paid → Finance Officer + Vendor
    notificationService.addNotification({
      notificationType: 'INVOICE_APPROVED',
      title: `Invoice Approved & Paid — ${target.invoiceNum}`,
      description: `Invoice ${target.invoiceNum} from ${target.vendorName} for ${toINR(target.amount)} has been processed and payment disbursed. Transaction reference: ${target.id}. Payment will reflect within 2–3 business days.`,
      relatedModule: 'Procurement',
  ...
  const NAV_ITEMS = [
    ...
    { id: "proc-invoices",          label: "  · Invoices",           icon: IndianRupee },
    ...
    { id: "rel-dashboard",        label: "Vendor Reliability",     icon: Shield },
    { id: "rel-details",          label: "  · Score Details",       icon: Activity },
    ...
    { id: "cc-repository",      label: "Contracts & Compliance",  icon: ClipboardList },
    { id: "cc-renewals",        label: "  · Renewal Dashboard",   icon: RotateCcw },
    { id: "cc-certifications",  label: "  · Certifications",      icon: FileBadge },
    { id: "cc-documents",       label: "  · Vendor Documents",    icon: FolderOpen },
    { id: "cc-compliance",      label: "  · Compliance Dashboard",icon: ShieldCheck },
    { id: "cc-notifications",   label: "  · Notifications",       icon: Bell },
    // ── Module 7: Communication Module ─────────────────────────────────────
    { id: "comm-messages",       label: "Communication",          icon: MessageSquare },
    { id: "comm-discussions",    label: "  · Discussions",        icon: MessageSquare },
    { id: "comm-history",        label: "  · Comm History",       icon: History },
    { id: "comm-files",          label: "  · File Sharing",        icon: FolderOpen },
    { id: "comm-activity",       label: "  · Activity Logs",       icon: Activity },
    // ── Module 8: Dashboard & Analytics ───────────────────────────────────
    { id: "pm-dashboard",        label: "PM Analytics Dashboard", icon: BarChart2 },
    { id: "vendor-dashboard",    label: "Vendor Portal Dashboard",icon: LayoutDashboard },
    { id: "admin-dashboard",     label: "Executive Admin Dashboard", icon: ShieldCheck },
  ...
  const renderNavTab = () => {
    ...
    switch (activeTab) {
      case "dashboard": {
        ...
        if (activeRole === "Procurement Manager") {
          return (
        ...
        if (activeRole === "Vendor") {
          const currentVendorObj = vendors.find(
        ...
        return (
          <div style={{ padding: "24px 28px" }}>
            ...
            {(activeRole as string) === "Administrator" && (
              <div>
            ...
            {(activeRole as string) === "Procurement Manager" && (
              <div>
                <div style={kpiGridStyle}>
                  <div style={cardStyle}>
      ...
      case "cc-repository":
      case "cc-renewals":
      case "cc-certifications":
      case "cc-documents":
      case "cc-compliance":
      case "cc-notifications": {
        return (
          <ContractManagement
            activeTab={activeTab === "contracts" ? "cc-repository" : activeTab}
            roleColor={currentRoleConfig.color}
            currentRole={activeRole}
            userName={userName}
      ...
      case "notifications":
        return (
          <NotificationsPage
            ...
            userName={userName}
            onNavigateTab={(tab: string) => setActiveTab(tab)}
            onSettings={() => setActiveTab("notifications-settings")}
          />
        );
      ...
      case "reports": {
        const currentVendorObjR = activeRole === "Vendor" ? (vendors.find(
          v => v.email.toLowerCase() === userEmail.toLowerCase() ||
               v.contacts.some(c => c.email.toLowerCase() === userEmail.toLowerCase())
        ) || vendors[0]) : undefined;
        return (
          <ReportsHub
            ...
            roleColor={currentRoleConfig.color}
            userName={userName}
            userVendorName={currentVendorObjR?.name}
          />
        );
      }
      case "invoices":
      ...
      case "rel-risk":
      case "rel-trends":
      case "rel-recommendations": {
        const currentVendorObj = activeRole === "Vendor" ? (vendors.find(
          v => v.email.toLowerCase() === userEmail.toLowerCase() ||
               v.contacts.some(c => c.email.toLowerCase() === userEmail.toLowerCase())
        ) || vendors[0]) : undefined;
        return (
          <VendorReliabilityComponent
            activeTab={activeTab}
            roleColor={currentRoleConfig.color}
            currentRole={activeRole}
            userVendorName={currentVendorObj?.name}
  ...
  return (
    <div style={{ display: "flex", fontFamily: "Inter, sans-serif", minHeight: "100vh", background: "#F9FAFB" }}>
      ...
      {/* Global Live Notification Toast Broker */}
      <NotificationToast />
